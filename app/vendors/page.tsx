...
import type { Vendor } from "@/lib/erp";
...
export default function VendorsPage() {
  ...
  function startInlineEdit(v: Vendor) {
  ...
  async function handleInlineSave() {
    ...
    if (isFormBlank(inlineForm)) {
      setInlineError("거래처명 등 하나 이상은 입력해주세요.");
      return;
    }
    setInlineError("");
    setInlineSaving(true);
    try {
      const updated = await api.updateVendor(inlineEditId, inlineForm);
  ...
  async function handleDelete(id: string) {
    if (!confirm("이 거래처를 삭제할까요? 관리자가 휴지통에서 복구할 수 있어요.")) return;
  ...
  const filtered = q
    ? vendors.filter(
        (v) =>
          v.name.includes(q) ||
          v.ceoName.includes(q) ||
          v.managerName.includes(q) ||
          v.phone.includes(q) ||
          v.mobile.includes(q)
      )
    : vendors;
    ...
    <div className="app">
      ...
      <div style={{ padding: "12px 16px 0 16px" }}>
        <div className="progress-auto-sub" style={{ textAlign: "left" }}>
          세금계산서를 주고받는 업체·개인 정보를 등록해두면, 장비·자재·운반비 집계 엑셀에 자동으로 정리돼요.
        </div>
      </div>

      <div className="wi-box" style={{ margin: "12px 16px 16px 16px" }}>
        <div className="wi-summary">새 거래처 등록</div>
        {error && (
      ...
      <div style={{ padding: "0 16px 10px 16px" }}>
        <input
          type="text"
          placeholder="🔍 거래처명·대표자·담당자·연락처로 조회"
          value={search}
      ...
      <div className="list" style={{ paddingBottom: 40 }}>
        {filtered.length === 0 ? (
          <div className="empty">{q ? "검색 결과가 없어요." : "등록된 거래처가 없어요."}</div>
        ) : (
          filtered.map((v) => {
                ...
                >
                  ...
                  <div className="row2">
                    <div className="field">
                      <label>대표자성명</label>
                      <input
                        value={inlineForm.ceoName}
                        onChange={(e) => setInlineForm({ ...inlineForm, ceoName: e.target.value })}
                      />
                    </div>
                    <div className="field">
                      <label>담당자성명</label>
                      <input
                        value={inlineForm.managerName}
                        onChange={(e) => setInlineForm({ ...inlineForm, managerName: e.target.value })}
                      />
                    </div>
                  </div>
                  <div className="row2">
            ...
            return (
              <div className="user-row" key={v.id}>
                <div>
                  <div className="uname">
                    {v.name || <span style={{ color: "#a09a89", fontWeight: 400 }}>(거래처명 미입력)</span>}{" "}
                    {v.ceoName && <span style={{ color: "#8a8371", fontWeight: 400 }}>· 대표 {v.ceoName}</span>}
                  </div>
                  <div className="uemail">
                    {v.bizRegNo || "사업자번호 미등록"}
                    {v.managerName ? ` · 담당 ${v.managerName}` : ""}
                    {v.phone || v.mobile ? ` · ${[v.phone, v.mobile].filter(Boolean).join(" / ")}` : ""}
