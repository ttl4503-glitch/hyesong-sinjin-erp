...
export async function POST(req: NextRequest) {
  ...
  if (!name && !bizRegNo && !ceoName && !managerName && !phone && !mobile && !bankName && !account && !email) {
    return NextResponse.json({ error: "거래처명 등 하나 이상은 입력해주세요." }, { status: 400 });
  }

  const vendor = await prisma.vendor.create({
    data: { name, bizRegNo, ceoName, managerName, phone, mobile, bankName, account, email },
  });
